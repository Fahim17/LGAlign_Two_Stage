...
def build_optimizer(cfg, model):
    ...
    params = model.trainable_modules.parameters()
...
# freeze_stage1_for_stage2() -- proj_satellite + stage2_satellite_qformer.
...
#              --resume-from-stage1) has already frozen Stage 1, built the
#              Stage 2 optimizer/scheduler, and is handing off directly into
#              Stage 2 -- in that case this loop skips Stage 1 entirely and
#              runs only the fixed-length Stage 2 block below. `optimizer`/
#              `scheduler` passed in are then the STAGE 2 ones already.
...
def train(cfg, model, train_loader, optimizer, scheduler, device,
    ...
    def run_stage2_epochs(stage2_optimizer, stage2_scheduler, global_epoch_offset):
        ...
        for stage2_local_epoch in range(stage2_epoch_count):
            ...
            model.trainable_modules["proj_satellite"].train()
            model.trainable_modules["stage2_satellite_qformer"].train()
...
# from that checkpoint, Stage 1 is frozen immediately, a fresh Stage 2
# optimizer/scheduler is built (the Stage 1 optimizer state in the
# checkpoint is intentionally NOT loaded, since it covers a different,
# now-frozen parameter set), and training continues from
# checkpoint_epoch + 1 using the GLOBAL epoch counter (so a checkpoint
# saved at epoch 19 resumes at epoch 20, not epoch 0). Stage 2 then runs
# for exactly (total_epochs - stage1_epochs) epochs regardless of what
# epoch the checkpoint came from.
# ============================================================================
def run_training(config_path: str = "configs/geo_dino_t5_qformer.json", resume_from_stage1: str = None):
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to a Stage 1 checkpoint. Skips Stage 1 entirely, freezes it, and runs Stage 2 "
             "for exactly (total_epochs - stage1_epochs) epochs, with global epoch numbering "
             "continuing from checkpoint_epoch + 1."
    )
    args = parser.parse_args()
    run_training(args.config, resume_from_stage1=args.resume_from_stage1)
